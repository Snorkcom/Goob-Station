...
{
    public sealed partial class VendingMachineMenu
    {
        private string GetItemText(EntityUid dummy, VendingMachineInventoryEntry entry)
        {
            var itemName = Identity.Name(dummy, _entityManager);

            // The marker means this vend will use a stored returned entity before falling back to spawning a new one.
            var returnedMarker = entry.HasReturnedItem ? " *" : string.Empty;
            return $"{itemName} [{entry.Amount}]{returnedMarker}";
        }
    }
}
